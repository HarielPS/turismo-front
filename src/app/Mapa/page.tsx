...
const center: [number, number] = latParam && lngParam
  ? [parseFloat(latParam), parseFloat(lngParam)]
  : [19.435272336896485, -99.13326147454349];
  const zoom = 13;
...
return (
  <div className="relative h-screen w-full overflow-hidden">
    ...
    <div className={`absolute h-full w-full sm:w-1/4 z-2 bg-amber-300 rounded-r-xl shadow-lg transition-all duration-300 transform ${isPanelVisible ? 'translate-x-0' : '-translate-x-full'}`}>
        <Infoevent servicio={selectedEvento} onClose={handlePanelToggle} />     </div>
    )}

    {/* Barra de búsqueda (posicionada sobre el mapa) */}
    <div className={`absolute z-4 w-full sm:w-2/3 top-5 transition-all duration-300 ${isPanelVisible ? 'sm:ml-[25%]' : 'sm:ml-4'}`}>
      <div className='flex flex-row'>
        <div className="relative top-0 h-full z-5">
          <ThemeLogo href="/" />
        </div>
